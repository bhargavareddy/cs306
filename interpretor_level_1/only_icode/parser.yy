...
assignment_statement:
	...
	IF '(' comp_stmt ')' GOTO BBNUM ';' ELSE GOTO BBNUM ';'
	{
	if (NOT_ONLY_PARSE)
	{
...
|
	...
	variable ASSIGN comp_stmt ';'
	{
	if (NOT_ONLY_PARSE)
	{
		int line = get_line_number();
		$$ = new Assignment_Ast($1, $3,line);
		$$->check_ast();
	}
	}
...
comp_stmt:
	comp_stmt GT comp_stmt
	{
	if (NOT_ONLY_PARSE)
	{
		$$ = new Comparison_Ast($1, $3, 1);
...
|  
	comp_stmt LT comp_stmt
	{
	if (NOT_ONLY_PARSE)
	{
		$$ = new Comparison_Ast($1, $3, 3);
...
|
	comp_stmt NE comp_stmt
	{
	if (NOT_ONLY_PARSE)
	{
		$$ = new Comparison_Ast($1, $3, 5);
...
|
	comp_stmt GE comp_stmt
	{
	if (NOT_ONLY_PARSE)
	{
		$$ = new Comparison_Ast($1, $3, 0);
...
|
   comp_stmt LE comp_stmt
	{
	if (NOT_ONLY_PARSE)
	{
		$$ = new Comparison_Ast($1, $3, 2);
...
|	
	comp_stmt EQ comp_stmt
	{
	if (NOT_ONLY_PARSE)
	{
		$$ = new Comparison_Ast($1, $3, 4);
